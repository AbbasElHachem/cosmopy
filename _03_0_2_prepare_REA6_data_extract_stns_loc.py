...
path_to_all_rea6_files = r'/run/media/abbas/EL Hachem 2019/REA6/Daily/REA6_data_daily'
...
for _year in list_years:
    print(_year)
    start_year = '01-01-%s 01:00:00' % _year
    end_year = '31-12-%s 01:00:00' % _year
    ...
    for grib_file in all_grib_files_year:
        ...
        if data_freq =='D':
            month_nbg = int(grib_file.split('.')[-3][-2:])
        # read grib file
...
#         tmp.shape
    ...
    df_pcp_model.to_csv(
        os.path.join(r'/run/media/abbas/EL Hachem 2019/REA6/Extracted_Hannover_Daily/pcp_%s.csv'
                     % _year),
        sep=';')
